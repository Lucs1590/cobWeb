...
while Linha <= 1400:
	...
	if str(nome_cien) in "*"+str(nome_cien_site)+"*":
		div = tree.xpath('id("N2")/table[1]/tr[3]/td/table/tr/td/table/tr[2]/td[2]/input/@value')
		cic = tree.xpath('id("N2")/table[1]/tr[3]/td/table/tr/td/table/tr[3]/td[2]/input/@value')
		rep = tree.xpath('id("N2")/table[1]/tr[3]/td/table/tr/td/table/tr[4]/td[2]/input/@value')
		habit = tree.xpath('id("N2")/table[1]/tr[3]/td/table/tr/td/table/tr[5]/td[2]/input/@value')
		adap = tree.xpath('id("N2")/table[1]/tr[3]/td/table/tr/td/table/tr[6]/td[2]/input/@value')
		alt = tree.xpath('id("N2")/table[1]/tr[3]/td/table/tr/td/table/tr[7]/td[2]/input/@value')

		fil = tree.xpath('id("N2")/table[1]/tr[4]/td/table/tr/td/table/tr[2]/td[2]/input/@value')
		for_limb = tree.xpath('id("N2")/table[1]/tr[4]/td/table/tr/td/table/tr[3]/td[2]/input/@value')
		sup = tree.xpath('id("N2")/table[1]/tr[4]/td/table/tr/td/table/tr[4]/td[2]/input/@value')
		consist = tree.xpath('id("N2")/table[1]/tr[4]/td/table/tr/td/table/tr[5]/td[2]/input/@value')
		nerv = tree.xpath('id("N2")/table[1]/tr[4]/td/table/tr/td/table/tr[6]/td[2]/input/@value')
		comp = tree.xpath('id("N2")/table[1]/tr[4]/td/table/tr/td/table/tr[7]/td[2]/input/@value')

		inf = tree.xpath('id("N2")/table[1]/tr[5]/td/table/tr/td/table/tr[2]/td[2]/input/@value')

		tip_fruto = tree.xpath('id("N2")/table[1]/tr[6]/td/table/tr/td/table/tr[2]/td[2]/input/@value')
		obs = tree.xpath('id("N2")/table[2]/tr/td/table/tr[1]/td/textarea/text()')
#--------------------------------------------------------TRATAMENTO DO XPATH------------------------------------------------------------------------
		if (len(str(div)) > 2) and (len(str(cic)) > 2) and (len(str(rep)) > 2) and (len(str(habit)) > 2) and (len(str(adap)) > 2) and (len(str(alt)) > 2) and (len(str(fil)) > 2) and (len(str(for_limb)) > 2) and (len(str(sup)) > 2) and (len(str(consist)) > 2) and (len(str(nerv)) > 2) and (len(str(comp)) > 2) and (len(str(inf)) > 2) and (len(str(tip_fruto)) > 2) and (len(str(obs)) > 2):
			div = str(div[0].strip('\n\t').encode('utf-8')) if div != [] else ""
			if div == '[]':
				div == 'NÃO REGISTRADO'
			cic = str(cic[0].strip('\n\t').encode('utf-8')) if cic != [] else ""
			if cic == '[]':
				cic = 'NÃO REGISTRADO'
			rep = str(rep[0].strip('\n\t').encode('utf-8')) if rep != [] else ""
			if rep == '[]':
				rep = 'NÃO REGISTRADO'
			habit = str(habit[0].strip('\n\t').encode('utf-8')) if habit != [] else ""
            #if habit == '[]':
            #	habit ='NÃO REGISTRADO'
			adap = str(adap[0].strip('\n\t').encode('utf-8')) if adap != [] else ""
			if adap == '[]':
				adap = 'NÃO REGISTRADO'
			alt = str(alt[0].strip('\n\t').encode('utf-8')) if alt != [] else ""
			if alt == '[]':
				alt = 'NÃO REGISTRADO'
			fil = str(fil[0].strip('\n\t').encode('utf-8')) if fil != [] else ""
			if fil == '[]':
				fil = 'NÃO REGISTRADO'
			for_limb = str(for_limb[0].strip('\n\t').encode('utf-8')) if for_limb != [] else ""
			if for_limb == '[]':
				for_limb = 'NÃO REGISTRADO'
			sup = str(sup[0].strip('\n\t').encode('utf-8')) if sup != [] else ""
			if sup == '[]':
				sup = 'NÃO REGISTRADO'
			consist = str(consist[0].strip('\n\t').encode('utf-8')) if consist != [] else ""
			if consist == '[]':
				consist = 'NÃO REGISTRADO'
			nerv = str(nerv[0].strip('\n\t').encode('utf-8')) if nerv != [] else ""
			if nerv == '[]':
				nerv = 'NÃO REGISTRADO'
			comp = str(comp[0].strip('\n\t').encode('utf-8')) if comp != [] else ""
			if comp == '[]':
				comp = 'NÃO REGISTRADO'
			inf = str(inf[0].strip('\n\t').encode('utf-8')) if inf != [] else ""
			if inf == '[]':
				inf = 'NÃO REGISTRADO'
			tip_fruto = str(tip_fruto[0].strip('\n\t').encode('utf-8')) if tip_fruto != [] else ""
			if tip_fruto == '[]':
				tip_fruto = 'NÃO REGISTRADO'
			obs = str(obs[0].strip('\n\t').encode('utf-8')) if obs != [] else ""
			if obs == '[]':
				obs = 'NÃO REGISTRADO'
